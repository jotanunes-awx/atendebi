...
async function main() {
  ...
  await prisma.integrationConfig.upsert({
    ...
    update: {
      ...
      settings: {
        mode: blipEnabled ? 'webhook + api backfill' : 'waiting-access',
        webhookSecretRequired: false,
        syncEnabled: process.env.BLIP_SYNC_ENABLED === 'true',
        syncStrategy: 'commands-api',
        apiBaseUrl: process.env.BLIP_HTTP_BASE_URL || process.env.BLIP_CONTRACT_ID || '',
