...
export function presentTicket(ticket: TicketWithRelations) {
  ...
  return {
    ...
    category: readString(metadata, 'categoryName', readString(metadata, 'group', 'Sem categoria')),
    callDurationMinutes: readNumber(metadata, 'durationMinutes', 0),
    ...
    isComplaint: readBoolean(
      riskFlags,
      'isComplaint',
      readBoolean(metadata, 'isComplaint', ticket.tags.some((ticketTag) => ticketTag.tag.name === 'Reclamacao')),
    ),
    isOpportunity: readBoolean(
      riskFlags,
      'isOpportunity',
      readBoolean(metadata, 'isOpportunity', ticket.tags.some((ticketTag) => ['Venda', 'Proposta'].includes(ticketTag.tag.name))),
    ),
    botFallback: readBoolean(riskFlags, 'botFallback', readBoolean(metadata, 'botFallback', false)),
    unresolved: readBoolean(
      riskFlags,
      'unresolved',
      readBoolean(metadata, 'unresolved', readString(metadata, 'resolutionStatus', '').toLowerCase().includes('nao solucionado')),
    ),
